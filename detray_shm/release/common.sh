...
# release rather than the traccc-aas.sif image. The release ships tritonserver,
# traccc, detray, vecmem, covfie and the Triton API packages, all built by one
# team with one compiler -- so the server ends up on exactly the stack Athena
# uses and the region's ABI gates pass by construction.
...
PRODUCER_SRC="${PRODUCER_SRC:-/tmp/${USER}/rel_producer_src}"
PRODUCER_BUILD="${PRODUCER_BUILD:-/tmp/${USER}/rel_producer_build}"
MODELS="${MODELS:-/tmp/${USER}/rel_models}"

REGION_NAME="${REGION_NAME:-/rel_itk_detector}"
